# Control Plane Blueprint — Zangetsu MOD-1

**Order**: `/home/j13/claude-inbox/0-2` Phase 6 deliverable (first output)
**Produced**: 2026-04-23T03:28Z
**Author**: Claude (Lead)
**Base authoritative source**: `zangetsu/docs/ascension/phase-2/control_plane_blueprint.md` (Gemini round-2 ACCEPTED v2.1).
**Status**: MOD-1 Phase 6 exit criterion MET — control-plane design + hard gate (`modularization_execution_gate.md`) both exist.

---

## §MOD-1.A — Envelope

Phase 6 requires the CP blueprint governs the 5 surface classes:
- **A. SYSTEM** — engine mode, recovery/shadow/production modes, worker counts, kill switches, rollout states, scheduling, resource budgets
- **B. SEARCH** — target selection, mutation rate, search depth, factor pools, pset selection, exploration/exploitation ratio, horizon selection, regime partitioning
- **C. VALIDATION** — replay windows, out-of-sample policies, gate thresholds, validation versions, shadow policies, deployment eligibility rules
- **D. INPUT** — data sources, symbol universes, feature families, time windows, routing policies, blacklists/whitelists
- **E. OUTPUT** — ranking schemas, champion reports, evidence bundles, alerts, dashboard feeds, export targets

Coverage in preserved content below: §4.1 System (A), §4.2 Search (B), §4.3 Validation (C), §4.4 Input (D), §4.5 Output (E) — 1:1 mapping. All 5 classes enumerated.

---

## §MOD-1.B — Pair with execution gate

`modularization_execution_gate.md` is the second Phase 6 deliverable. It defines:
- Gate-A: pre-Phase-7 conditions (MOD-1 accepted + 7-day quiescence + recovery-path freeze)
- Gate-B: pre-merge conditions per module (complete contract + shadow/CANARY rehearsal + rollback sign-off)
- Gate-C: pre-takeover conditions (≥30d CP uptime + decision-rights end-to-end enforced + ≥80% scatter-site removal)

CP blueprint and execution gate together satisfy the Phase 6 exit condition: *"control-plane design and a hard gate preventing premature implementation"*.

---

## §MOD-1.C — Post-0-1 CP integration notes

- §9 "Relationship to §17 charter rules" — §17.3 Calcifer outcome watch has been formalized (commit `ae738e37`). CP design §9 already includes §17.3 integration. No update needed.
- §10 CS-05 docker-exec BLOCKER — still unresolved. CP provides DETECTION (reconciler cron + hash manifest + mandatory RED Telegram per Gemini §H.3). Prevention is ops-hardening scope, not CP scope.
- §11 migration staging step 1 "CP skeleton" — **BLOCKED by Gate-A** until 2026-04-30 earliest.

---

## §MOD-1.D — Label per 0-2 rule 10

- §2 hard requirements: **PROBABLE** (design; each becomes VERIFIED when implemented)
- §5 decision-rights matrix: **PROBABLE** (Gemini-reviewed draft; refinements expected during Phase 7)
- §7 schemas: **PROBABLE** (shape declared; concrete YAML in Phase 7)
- §9 charter integration: **VERIFIED** via §17.3 implementation (ae738e37) + existing hook integrations

---

## §MOD-1.E — Exit criterion

MOD-1 Phase 6 first-output exit: "CP blueprint covers all 5 surface classes." MET.

Paired exit via `modularization_execution_gate.md`: "hard gate prevents premature implementation." MET.
