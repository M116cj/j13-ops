# Architecture Drift Map — Zangetsu MOD-1

**Order**: `/home/j13/claude-inbox/0-2` Phase 3 deliverable
**Produced**: 2026-04-23T03:12Z
**Author**: Claude (Lead)
**Base authoritative source**: `zangetsu/docs/ascension/phase-1/architecture_drift_map.md` (Gemini round-2 ACCEPTED v2.1).
**Status**: MOD-1 Phase 3 exit criterion MET — the gap between intended and actual is explicit, with severity + root cause + remediation phase per drift.

---

## §MOD-1.A — Envelope

23 drift entries (D-01..D-23) are preserved from the base doc below. Two BLOCKERS, 11 HIGH, 9 MEDIUM, 3 LOW. Every entry has:
- Layer/contract it violates
- Intended vs Actual
- Severity
- Root cause
- Remediation phase

MOD-1 Phase 3 required coverage (in-scope per 0-2):
- intended vs actual contradictions ✅ (each D entry)
- missing layers ✅ (D-01 CP; D-12 L10 adapter)
- mixed responsibilities ✅ (D-02 kernel; D-08 gates; D-18 admission+kernel)
- scattered configs ✅ (D-03 thresholds; D-11 governance; linked `scattered_config_map.md`)
- uncontrolled IO ✅ (D-16 XPD; linked `uncontrolled_io_map.md`)
- invisible state transitions ✅ (D-18; D-21 schema-hash missing)
- governance drift ✅ (D-11; D-17 hooks integrity)
- observability drift ✅ (D-10 static-only; D-19 VIEW consumer contract)
- modularization blockers ✅ (D-01 BLOCKER + CS-05 BLOCKER)

---

## §MOD-1.B — Post-0-1 drift updates

### D-01 Control Plane missing — UPDATE
Status at 2026-04-23: **UNCHANGED — CP still does not exist**. MOD-1's `control_plane_blueprint.md` defines the design; Phase 7 (gated by `modularization_execution_gate.md` Gate-A) will build it.

### D-07 Search is GP-only — UPDATE
Additional evidence from 0-1: since arena is now FROZEN, the practical impact is paused. The structural drift remains; when restart triggers (Path-A/B/C from `track3_restart_memo.md`), having only GP available will re-surface this drift.

### D-11 Governance rules in human discipline — PARTIAL RESOLUTION
Commit `ae738e37` formalized Calcifer §17.3 outcome-watch code (previously HUMAN-executed). D-11 severity downgraded to MEDIUM post-0-1 because one of the §17.3 / §17.4 / §17.5 / §17.6 / §17.7 enforcement gaps has been closed (§17.3 is now code-enforced).

Updated drift roll-up:

| Severity | Before 0-1 | After 0-1 |
|---|---:|---:|
| BLOCKER | 2 | 2 (unchanged — CP + docker-exec) |
| HIGH | 11 | 10 (D-11 → MEDIUM) |
| MEDIUM | 9 | 10 |
| LOW | 3 | 3 |

### D-23 cold-start as workaround-turned-default — REMAINS SYMPTOM
Arena frozen means cold-start is frozen too. Still a SYMPTOM of 60-bar formulation problem, not an independent drift. Remediation tracks into Path-A (D1 target-gate redesign).

### NEW (tracked for MOD-1 completeness)

**D-24 — d-mail-miniapp / calcifer-miniapp outside version control**
- Layer: L7 + governance hygiene
- Intended: all production code under VCS with PR review path
- Actual: 2 production FastAPI services (d-mail-miniapp 1047-line server, calcifer-miniapp 38KB server.py) have NO .git; history = `server.py.bak_vNN_YYYYMMDD_HHMMSS` filenames
- Severity: CRITICAL (d-mail-miniapp hosts Claude Command Center + ops stack)
- Root cause: miniapps evolved as ad-hoc ops tools; never formalized
- Remediation: B3/B4 infra blocker (pending j13 repo owner decision)

**D-25 — arena13_feedback.py silent KeyError every 5 minutes**
- Layer: L4 guidance feedback + L8.G integrity
- Intended: guidance loop writes `a13_guidance.json` every 5min; A1 reads it for weight adjustments
- Actual: cron entry invokes it, script imports `settings.py` which requires `ZV5_DB_PASSWORD` env; cron shell has no such env → KeyError at import → log-only failure
- Severity: MEDIUM (during freeze); HIGH (when arena restarts, silent stale guidance continues)
- Root cause: cron env ≠ systemd env, never reconciled; `settings.py` has no fallback
- Remediation: Phase 7 migration to systemd timer with `EnvironmentFile=/home/j13/.env.global`; paired with §MOD-1 Gate-B enforcement

**D-26 — Gemini CLI adversarial reviewer failure surface (Mac-side)**
- Layer: meta — not an engine layer, but impacts Q1 adversarial coverage for EVERY module contract review
- Intended: Gemini CLI available for Mandatory review before arch changes (CLAUDE.md §5)
- Actual: `gemini` CLI on Mac fails at keytar + EPERM `.Trash` + silent exit after MCP refresh (verified during 0-1 Phase B)
- Severity: MEDIUM (workflow-impacting; Claude fallback to self-adversarial voice per §1 "two voices")
- Root cause: keytar.node ABI mismatch after Node upgrade (probable)
- Remediation: `brew reinstall gemini-cli` + probably Full Disk Access grant

---

## §MOD-1.C — Modularization blockers (MOD-1 Phase 3 required focus)

Per 0-2 Phase 3 required contents, the **BLOCKER list for modularization** at MOD-1 completion:

| ID | Blocker | Why blocks modularization |
|---|---|---|
| D-01 | Control Plane missing | L1 is the foundation of every other layer's parameter lookup; migrating any module without CP creates a circular dependency |
| CS-05 | docker-exec bypasses code gates (per `uncontrolled_io_map.md §CS-05`) | Any module contract's "state_schema" is unreliable if unmitigated docker-exec can rewrite DB without audit |
| D-24 | miniapps off VCS | Migration PRs can't review what isn't in a repo |

These three are the LITERAL Gate-A.1 rejection conditions in `modularization_execution_gate.md §2`.

---

## §MOD-1.D — Label per 0-2 rule 10

- §1 reading rules: design-time, no label needed
- §2 drift entries D-01..D-23: **VERIFIED** (each backed by code citation or N1.x evidence)
- §2 drift entries D-24..D-26 (this file): **VERIFIED** (0-1 Phase C audit)
- §3 severity roll-up: **PROBABLE** (severity is judgment within defined rubric)
- §4 confidence labels: already scoped

---

## §MOD-1.E — Exit criterion

MOD-1 Phase 3 exit: *"The team can explain exactly why the current system is not yet a coherent modular engine."*

Met by: 26 drift entries + severity roll-up + §MOD-1.C explicit BLOCKER list for modularization. The reason is not a single cause; it is a structural pattern of missing CP + script-centric implementation + scattered configs + human-discipline governance + off-VCS miniapps.

Proceed to Phase 4 (`module_boundary_map.md` + `module_contract_template.md`).
