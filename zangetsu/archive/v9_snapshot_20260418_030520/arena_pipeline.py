...
from zangetsu.services.bloom_service import bloom_init as rbloom_init, bloom_add as rbloom_add, bloom_madd as rbloom_madd, bloom_count as rbloom_count

# V9: optional Bayesian-guided search (graceful fallback if unavailable)
try:
    from zangetsu.services.v9_search import V9SearchCoordinator
    _V9_SEARCH_AVAILABLE = True
except Exception as _e:
    _V9_SEARCH_AVAILABLE = False
    V9SearchCoordinator = None
...
async def main():
    ...
    # V9: optional Bayesian-guided search coordinator
    _v9_coordinator = None
    if _V9_SEARCH_AVAILABLE and os.environ.get("A1_USE_V9_SEARCH", "1") == "1":
        try:
            storage_url = f"postgresql://zangetsu:{os.environ.get('ZV5_DB_PASSWORD', '9c424966bebb05a42966186bb22d7480')}@127.0.0.1:5432/zangetsu"
            _v9_coordinator = V9SearchCoordinator(
                indicator_pool=DIRECTIONAL,
                period_choices=[14, 20, 30, 50],
                storage_url=storage_url,
                study_name=f"zv9_a1_w{worker_id}",
            )
            log.info(f"V9 search coordinator active: worker={worker_id}")
        except Exception as e:
            log.warning(f"V9 search init failed, falling back to random: {e}")
            _v9_coordinator = None
            ...
            for period in period_choices:
                try:
                    ...
                    try:
                        ...
                        vals = None
                    if vals is None:
                        vals = zi.compute(nm, {"period": period}, np.ascontiguousarray(close, dtype=np.float64), np.ascontiguousarray(high, dtype=np.float64), np.ascontiguousarray(low, dtype=np.float64), np.ascontiguousarray(vol, dtype=np.float64)) if rust else np.zeros(len(close))
                    mad = np.median(np.abs(vals - np.median(vals)))
                    if mad > 0:
            ...
            for period in period_choices:
                try:
                    ...
                    try:
                        ...
                        vals = None
                    if vals is None:
                        vals = zi.compute(nm, {"period": period}, np.ascontiguousarray(qclose, dtype=np.float64), np.ascontiguousarray(qhigh, dtype=np.float64), np.ascontiguousarray(qlow, dtype=np.float64), np.ascontiguousarray(qvol, dtype=np.float64)) if rust else np.zeros(len(qclose))
                    mad = np.median(np.abs(vals - np.median(vals)))
                    if mad > 0:
    ...
    while running:
        ...
        best_rec = None  # V9: track which V9 recommendation produced best_champion
        ...
        for c in range(200):
            rec = None  # V9: reset per-iteration recommendation handle
            ...
            # ── V9: Bayesian-guided suggestion (with graceful fallback to random) ──
            names = None
            periods = None
            if _v9_coordinator is not None:
                try:
                    rec = _v9_coordinator.next_combo(regime=regime)
                    names = [ind for ind, _ in rec.indicators]
                    periods = [p for _, p in rec.indicators]
                    if not names:
                        rec = None
                        names = None
                        periods = None
                except Exception as e:
                    log.debug(f"V9 suggest failed, random fallback: {e}")
                    rec = None
                    names = None
                    periods = None

            if names is None:
                # Legacy random path (fallback)
            ...
            if score > best_score:
                ...
                best_arrs = arrs_full
                best_rec = rec  # V9: capture the recommendation that produced this champion (may be None if fallback path)
        ...
        if bt_a2.total_trades < 25 or _a2_pos < 2:
            ...
            # V9: report rejection back to coordinator (negative signal)
            if _v9_coordinator is not None and best_rec is not None:
                try:
                    _reject_score = float(best_score) - 1.0  # penalty signal
                    _v9_coordinator.report_result(best_rec, a3_score=_reject_score, passed_a3=False)
                except Exception as _ve:
                    log.debug(f"V9 report_result (rejected) failed: {_ve}")
            continue
        ...
        # V9: report successful champion back to coordinator (positive signal)
        if _v9_coordinator is not None and best_rec is not None:
            try:
                _v9_coordinator.report_result(best_rec, a3_score=float(best_score), passed_a3=True)
            except Exception as _ve:
                log.debug(f"V9 report_result (passed) failed: {_ve}")
