...
for f in signal_utils backtester; do
  cp ~/j13-ops/zangetsu/engine/components/${f}.py.bak ~/j13-ops/zangetsu/engine/components/${f}.py
done
for f in arena_pipeline arena23_orchestrator arena45_orchestrator; do
  cp ~/j13-ops/zangetsu/services/${f}.py.bak ~/j13-ops/zangetsu/services/${f}.py
done
mv ~/j13-ops/zangetsu/engine/components/composite_score.py.deleted ~/j13-ops/zangetsu/engine/components/composite_score.py
bash ~/j13-ops/zangetsu/zangetsu_ctl.sh restart
```
