...
class HealthMonitor:
    ...
    async def _handle_metrics(self, request: Any) -> Any:
        ...
        data = self.collect_all()
        lines = [
            "# HELP zangetsu_v5_uptime_seconds Engine uptime",
            "# TYPE zangetsu_v5_uptime_seconds gauge",
            "zangetsu_v5_uptime_seconds " + str(data["uptime_s"]),
        ]

        for metric_name, value in self._custom_metrics.items():
            safe_name = metric_name.replace(".", "_").replace("-", "_")
            lines.append("zangetsu_v5_" + safe_name + " " + str(value))
