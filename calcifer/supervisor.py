...
TOOLS = {
    ...
    "check_db_health": lambda: run_cmd("docker exec deploy-postgres-1 psql -U zangetsu -d zangetsu -t -c \"SELECT 'rows=' || count(*) || ' deployable=' || count(*) FILTER (WHERE status='DEPLOYABLE') FROM champion_pipeline_fresh\" 2>&1"),
